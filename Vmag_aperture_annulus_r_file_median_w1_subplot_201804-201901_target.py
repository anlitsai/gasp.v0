...
file_info='gasp_target_fitsheader_info_exclude_baddata_201804-201901.txt'
...
dir_obj='Vmag_InstMag/annu_w1_201804/'+obj_name+'/'
...
idx_fitsheader=df_info[(df_info['Object']==obj_name) & (df_info['FilterName'].str.contains(filter_ID))].index
...
for i in idx_fitsheader_canfit:
    ...
    fits_calib=fits_root+'_calib.new'
...
#    dir_file=yearmonth+'/slt'+date+'_calib_sci/'
    dir_file='data_201804-201901_wcsfixed/slt'+date+'_calib_sci/'
...
#   sys.exit(0)
    ...
    j=0
    vmag[0]=-9999.
    vmag_fitting[0]=vmag[0]
    print()
    print('j final_sum         mag_instrument      Vmag')
    for j in range(n_refstar):
        mag_instrument[j]=-2.5*np.log10(bg_subtract_sum[j])
        print(j, final_sum[j],mag_instrument[j],vmag[j])
...
#    SN=bkg_mean/bkg_std ??    
#    ErrorVmag=2.5*log10(1+N/S)=1.086/(S/N)
#    signal=final_sum
#    noise=bkg_std*aperture_pix[i].area
#    mag_err=1.086/snr

    
#    Vmag_err_targets[k]=mag_err[0]
...
#    plt.text(x0*0.95,y0*0.93,text_vmag+text_vmag_err,fontsize=font_small)
...
#    plt.show()    
#    plt.savefig(dir_obj+'Vmag_InstMag_aper_'+obj_name+'_'+date+'_fwhm'+str(xfwhm)+'_'+str(k)+'.png')
#    plt.savefig(dir_obj+'Vmag_InstMag_aper_'+obj_name+'_annu_'+date+'_'+str(k)+'.png')
#    plt.close()
...
#    plt.figure()
#    plt.figure(figsize=(8,6))
#    plt.scatter(mag_instrument[k][1:],vmag[1:])
#    plt.plot(mag_instrument[k][1:],vmag[1:],'o')

    d_vmag_fitting=vmag[1:]-vmag_fitting[k][1:]
    print('... fitting residual:',d_vmag_fitting)

#    vmag=shift+slope*mag_instrument
#    mag_instrument_fitting_refstar=(vmag_fitting[k][1:]-shift)/slope
...
#        plt.errorbar(mag_instrument[k][0],vmag_fitting[k][0],yerr=mag_instrument_err[k][0],fmt='o',color='red',label=label_target)
#    else:
#        plt.errorbar(mag_instrument[k][0],vmag_fitting[k][0],yerr=0.2,uplims=True,fmt='o',color='red',label=label_target)        
#       plt.plot(mag_instrument[k][1:],vmag[1:],'o',label='Ref.Star')
#        plt.plot(mag_instrument[k][0],vmag_fitting[k][0],'o',color='red',label=label_target)
#    else:
#        plt.errorbar(mag_instrument[k][0],vmag_fitting[k][0],yerr=0.2,uplims=['True'],marker='o',color='red',label=label_target)
#    plt.plot(mag_instrument[k][1:],vmag_fitting[k][1:],'o',color='steelblue',label=label_source[1:])
#    plt.errorbar(mag_instrument[k][0],vmag_fitting[k][0],yerr=mag_instrument_err[k][0],fmt='o',color='red',label=label_target)
#    plt.errorbar(mag_instrument_source,vmag_source,yerr=mag_err[:-1])
...
#    print(mag_instrument,Vmag_targets)
...
'''
df_Vmag_keep=df_out_keep[['JD','Vmag','ErrorVmag']].astype(float) #.reset_index(drop=True)
#df_Vmag_keep=df_out_keep[['JD','Vmag','ErrorVmag']].reset_index(drop=True)
#df_Vmag_keep['JD']=df_Vmag_keep['JD'].map('{:.4f}'.format)
df_Vmag_keep['Observatory']='Lulin (SLT)'

print(df_Vmag_keep)

#file_Vmag_out_keep=dir_obj+'Vmag_'+obj_name+'.txt'
file_Vmag_out_keep=dir_obj+'g'+iau_name_short+'r_LuS_'+191126+'.dat'
#df_Vmag_keep.to_csv(file_Vmag_out_keep,sep=',',index=False,float_format='%.5f')
#df_Vmag_keep.to_csv(file_Vmag_out_keep,sep='\t',index=False,float_format='%.5f')
#with open(file_Vmag_out_keep,'w') as f: file_Vmag_out_keep.to_string(f,col_space=3)
df_Vmag_keep.to_csv(file_Vmag_out_keep,sep='\t',index=False,float_format=['%.5f','%.3f','%.3f'],header=None)
'''
...
#	print(i)
#	plt.scatter(JD,vmag_fitting[:,i],label=label_source[:,i],fmt='o')
#        plt.scatter(JD,vmag_fitting[:,i],label=str(refstarID[i]),marker='o')
#        plt.axhline(y=vmag[1:])
#	plt.axhline(JD,vmag[1:],linestyle='-')
#	for j in range(k):
#	    plt.scatter(JD,vmag_fitting[k][i],label=label_source[k][i],fmt='o')
...
plt.ylabel('Vmag',fontsize=font_middle)
plt.gca().invert_yaxis()
#axs[1].set_title(obj_name) #,fontsize=font_small)
...
#plt.show()
plt.savefig(dir_obj+'Vmag_JD_'+obj_name+'.png',dpi=120)
plt.close()
'''
# -----------------------------------
