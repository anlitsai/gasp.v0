...
'''
#cmd_search_flat_filter="find ./ |grep "+dir_month+" |grep fts | grep AutoFlat | cut -d / -f5 | awk -F'_Astro' '{print $1}'| rev | cut -c1-3 | rev | cut -d - -f2 | sort | uniq"
cmd_search_flat_filter="find ./ |grep "+dir_month+" |grep GASP| grep fts | rev | cut -d - -f1 | cut -d _ -f3 | rev| sort | uniq"
print(cmd_search_flat_filter)
...
for i in list_flat_filter:
    cmd_search_file_flat='find ./ |grep '+dir_month+' | grep fts | grep AutoFlat | grep '+i+'_Astrodon'
