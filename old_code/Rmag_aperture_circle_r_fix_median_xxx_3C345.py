...
dir_obj='Rmag_InstMag/'+obj_name+'/circle_r=fwhmx'+str(xfwhm)+'/'
...
#   sys.exit(0)
    ...
    j=0
    value_before_calculation=-9999.
    rmag[0]=value_before_calculation

    print()
    print('j final_sum       mag_instrument_object  Rmag_object')
    ...
    mag_instrument_object=mag_instrument[:n_refstar-1]
    rmag_object=rmag[:n_refstar-1]
...
#   print('b =','%.3f' %b)
#   print('m =','%.3f' %m)
    text_fitting='Rmag = '+str('%.3f' %b)+' + '+str('%.3f' %m)+' * (Instrument Magnitude)'
    print(text_fitting)
    
    rmag_fitting_object=b+m*mag_instrument_object
    
    Rmag_targets[k]=rmag_fitting_object[0]
    rmag[0]=rmag_fitting_object[0]
...
#    text_fig=text_rmag+', '+text_fwhm
    ...
    plt.text(x0*0.95,y0*0.93,text_rmag,fontsize=12)
...
#    plt.show()    
    ...
    rmag_min=min(rmag_object)
    rmag_max=max(rmag_object)
    imag_min=min(mag_instrument_object)
    imag_max=max(mag_instrument_object)
    
    plt.figure()
#    plt.scatter(mag_instrument_refStar,rmag_refStar)
#    plt.plot(mag_instrument_refStar,rmag_refStar,'o')
    plt.plot(mag_instrument_target,rmag_target,'o',label='Target',color='red')
    plt.plot(mag_instrument_refStar,rmag_refStar,'o',label='ref.Star',color='blue')
    plt.plot(mag_instrument_object,rmag_fitting_object,'-',label='lightcurve',color='grey')
    plt.xlabel('Instrument Magnitude')
    plt.ylabel('Rmag')
    plt.title(figtitle)
#    text_fig='Rmag='+str('%.3f' %rmag_object)+', fwhm='+str('%.3f' %fwhm[i])
    plt.text(mag_instrument_object-0.5,rmag_object-0.5,obj_name,fontsize=12,color='red')
    plt.text(mag_instrument_object-0.5,rmag_object-0.8,text_rmag,fontsize=12)
    plt.text(mag_instrument_object-0.5,rmag_object-1.1,text_fwhm,fontsize=12)
    plt.text(mag_instrument_object-0.5,rmag_object-1.4,text_radius,fontsize=12)
    plt.text(mag_instrument_object+0.5,rmag_object-0.5,text_fitting,fontsize=12)
#    plt.text(mag_instrument_object*0.9,rmag[0]*0.9,'Rmag='+str(rmag[0]))
...
#    plt.show()
    ...
    plt.close()    
    print('new Rmag_targets =',rmag_target)
...
print(df_out)
file_Rmag_out=dir_obj+'Rmag_aperture_'+obj_name+'.txt'
...
plt.legend(loc='best')
#plt.show()
plt.savefig(dir_obj+'Rmag_JD_'+obj_name+'_line.png')
plt.close()
#sys.exit(0)
