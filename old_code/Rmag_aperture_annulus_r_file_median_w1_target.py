...
ID_selected=np.array([986,987])   # 3C279
...
#    print(WCS.world_axis_physical_types)
    ...
    for j1 in range(n_refstar):
        print('idx',i, '-',j1+1,'/',n_refstar,', Ref.Star:',refstarID[j1],') #', k+1,'/',n_idx,'image' )
...
#        for mask in annulus_masks:
        ...
        print('... bkg_sum (statistic) =','%.2f' %bkg_sum)
#        bkg_median.append(median_sigclip)
#        bkg_median = np.array(bkg_median)
        bkg_sum=aper_annu_sum1/area_sum1*area_sum0
        print('... bkg_sum (annulus)   =','%.2f' %bkg_sum)
#        print()
...
#    print('residual =', residual)
    ...
    rmag_fitting=shift+slope*mag_instrument[k]
    print('... rmag_fitting (Ref.Stars only):',rmag_fitting[1:])
    print('... rmag_fitting (Target only):',rmag_fitting[0])
    
    Rmag_targets[k]=rmag_fitting[0]
...
#    if change ra,dec of reference star, run 'gasp_refStar_radec.py again.    
    ...
    for i in range(n_refstar):
        ...
        if i==0:
            text_rmag=str(refstarID[i])+', Rmag='+str('%.3f' %rmag_fitting[0])
#            print(text_rmag)
            aperture_pix[i].plot(color='red', lw=1)
            plt.text(ra_pix[i]*1.02,dec_pix[i]*0.98,text_rmag,fontsize=font_small,color='red')
        else:
            aperture_pix[i].plot(color='white', lw=1)
            plt.text(ra_pix[i]*1.02,dec_pix[i]*0.98,str(refstarID[i]),fontsize=font_small)
...
#    text_rmag='Rmag='+str('%.3f' %rmag_fitting[0])
...
#    plt.text(x0*0.95,y0*0.90,text_rmag,fontsize=font_small,color='red')
#    for i in range(n_refstar):
...
#    plt.plot(mag_instrument[k][1:],rmag[1:],'o')
    ...
    d_rmag_fitting=rmag[1:]-rmag_fitting[1:]
    print('... fitting residual:',d_rmag_fitting)

#    rmag=shift+slope*mag_instrument
#    mag_instrument_fitting_refstar=(rmag_fitting[1:]-shift)/slope
...
#    SSregression_rmag=sum((rmag_fitting[1:]-np.mean(rmag[1:]))**2)
    ...
    plt.plot(mag_instrument[k],rmag_fitting,'-',color='grey',label=text_fitting)
#    plt.plot(mag_instrument[k][1:],rmag[1:],'o',label='Ref.Star')
    ...
    label_obj=obj_name+', Rmag='+str('%.2f' %rmag_fitting[0])+'$\pm$'+str('%.2f' %err_mag_total_per_img[k])
    plt.plot(mag_instrument[k][0],rmag_fitting[0],'o',color='red',label=label_obj)
#    plt.errorbar(mag_instrument[k][0],rmag_fitting[0],yerr=mag_instrument_err[k][0],fmt='o',color='red',label=label_obj)
...
#    plt.show()
    ...
    plt.close()    
#    print('target new Rmag =',rmag_fitting[0])
...
file_Rmag_out_simple=dir_obj+'Rmag_'+obj_name+'.txt'
