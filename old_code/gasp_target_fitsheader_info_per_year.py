...
#folder=sys.argv[1]
#folder='slt20190822'
print('... will grap target information for one year ...')
dir_year='slt'+year
...
file_info='gasp_target_fitsheader_info_'+dir_year+'.txt'
if os.path.exists(file_info):
    os.remove(file_info)
f_info=open(file_info,'w')

file_log='gasp_target_fitsheader_info_'+dir_year+'.log'
...
cmd_search_file_sci="find ./|grep "+dir_year+"|grep 'fits\|fts'|grep GASP |sort "
...
#    print(info_sci)
    ...
    info_write=str(idx)+'|'+date_obs+'|'+ str(filename_sci)+'|'+str(obj)+'|'+ra_hhmmss+'|'+dec_ddmmss+'|'+str('%.4f' %ra_deg)+'|'+str('%.4f' %dec_deg)+'|'+str('%.4f' %ra_pix)+'|'+str('%.4f' %dec_pix)+'|'+filter_name+'|'+str(jd)+'|'+str(exptime)+'|'+str('%.4f' %zmag)+'|'+str('%.4f' %fwhm)+'|'+str('%.4f' %altitude)+'|'+str('%.4f' %airmass)
