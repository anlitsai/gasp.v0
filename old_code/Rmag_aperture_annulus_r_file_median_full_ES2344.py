...
file_info='gasp_target_fitsheader_info_exclude_baddata.txt'
print('... will read '+file_info+' ...')
df_info=pd.read_csv(file_info,delimiter='|')
#print(df_info)

colnames_info=list(df_info.columns)
# ['ID', 'DateObs', 'Filename', 'Object', 'RA_hhmmss', 'DEC_ddmmss',
# 'RA_deg', 'DEC_deg', 'RA_pix', 'Dec_pix', 'FilterName', 'JD',
# 'ExpTime_sec', 'Zmag', 'FWHM', 'Altitude', 'Airmass']
...
dir_obj='Rmag_InstMag/annu/'+obj_name+'/'
...
#print(df_refstar)
colnames_refstar=list(df_refstar.columns)
# ['Unnamed: 0', 'IAU_Name', 'ObjectName', 'RA_hhmmss', 'DEC_ddmmss', 'Redshift',
# 'RefStarregname', 'RefStarID', 'RefStarRA_hhmmss', 'RefStarDEC_ddmmss',
# 'RefStarRA_deg', 'RefStarDEC_deg', 'R_circle_as',
# 'R_inner_as', 'R_outer_as', 'Rmag', 'Rmag_err', 'NoteIdx', 'Note']
...
# ['ID', 'DateObs', 'Filename', 'Object', 'RA_hhmmss', 'DEC_ddmmss',
# 'RA_deg', 'DEC_deg', 'RA_pix', 'Dec_pix', 'FilterName', 'JD',
# 'ExpTime_sec', 'Zmag', 'FWHM', 'Altitude', 'Airmass', 'Redshift',
# 'RefStarregname', 'RefStarID', 'RefStarRA_hhmmss', 'RefStarDEC_ddmmss',
# 'RefStarRA_deg', 'RefStarDEC_deg', 'RefStarRA_pix', 'RefStarDEC_pix', 
# 'Rmag', 'Rmag_err',
# 'R_circle_as', 'R_inner_as', 'R_outer_as',
## 'R_circle_pix', 'R_inner_pix', 'R_outer_pix',
## 'R_circle_area_pix2', 'R_annlus_area_pix2', 'R_outer_pix',
# 'AperCircleCounts', 'ErrAperCircleCounts',
# 'AperAnnlusCount', 'ErrAperAnnlusCount',
# 'AperBGsubCounts', 'ErrAperBGsubCounts',
# 'InstMag', 'ErrInstMag'
# 'slope', 'shift','R_square',
# 'ErrorCountsBGsub_perimg', 'ErrorFitting','ErrorRmag'
#  'Rmag_fitting',  'd_Rmag_err'
# 
...
#ID_selected=np.array([794,795,796,797,1319,1320,1421,1422])   # 4C38-41
#ID_selected=np.array([794,795,796,797])   # 4C38-41
#ID_selected=np.array([1319,1320,1421,1422])   # 4C38-41
#ID_selected=np.array([1036,1037])   # ES2344+514
#ID_selected=np.array([1037,1038])   # ES2344+514
ID_selected=np.array([1438,1439,1440,1441])   # ES2344+514
...
err_count_BGsub_per_img=np.array([0.]*n_idx)
...
file_reg='RefStar_'+obj_name+'_annu_mod_fk5.reg'


cmd_search_reg_radecr='cat '+dir_reg+file_reg+'| grep "circle" |cut -d "(" -f2 | cut -d ")" -f1'
print(cmd_search_reg_radecr)
list_reg_radecr=os.popen(cmd_search_reg_radecr,"r").read().splitlines()
print(list_reg_radecr)
n_reg=len(list_reg_radecr)

for i in range(n_reg):
    radecr=list_reg_radecr[i].split(',',3)
    ra=radecr[0]
    dec=radecr[1]
    r_circle=radecr[2].split('"',2)[0]
    print(ra,dec,r_circle)

cmd_search_reg_rinnout='cat '+dir_reg+file_reg+'| grep "annulus" |cut -d "(" -f2 | cut -d ")" -f1'
print(cmd_search_reg_rinnout)
list_reg_rinnout=os.popen(cmd_search_reg_rinnout,"r").read().splitlines()
print(list_reg_rinnout)

for i in range(n_reg):
    rinnout=list_reg_rinnout[i].split(',',4)
#    ra=radecr[0]
#    dec=radecr[1]
    r_inner=rinnout[2].split('"',2)[0]
    r_outer=rinnout[3].split('"',2)[0]
...
for i in idx_fitsheader:
    print('=================================')
    print('idx',i, ') ID =',ID[i],', #', k+1,'/',n_idx,'image')
...
#    print(WCS.world_axis_physical_types)
    ...
    for j1 in range(n_refstar):
        print('idx',i, '-',j1+1,'/',n_refstar,'Ref.Star ) #', k+1,'/',n_idx,'image' )
...
#        for mask in annulus_masks:
        ...
        bkg_sum=median_sigclip*aperture_pix[j1].area
...
#        print(phot_annu_table)        
        ...
        print('... mag_instrument =','%.4f' %mag_instrument[k][j1])
        ...
        print('... mag_instrument_sum+/-err =','%.4f' %mag_instrument_sum_err[k][j1])
...
#    print(bg_subtracted_err[k])
    err_count_BGsub_per_img[k]=np.sqrt(sum(bg_subtracted_err[k]**2)/n_refstar)
    print('... measurement error after background subtraction (counts):','%.4f' %err_count_BGsub_per_img[k])  
...
#    print('annulus sum error',aper_sum_err1[k])
    ...
    mag_instrument_refstar=mag_instrument[k][1:n_refstar]
    rmag_refstar=rmag[1:n_refstar]
#    print(mag_instrument_refstar)
#    print(rmag_refstar)
...
#   sys.exit(0)
    ...
    mag_instrument_refstar=mag_instrument[1:n_refstar]
    rmag_refstar=rmag[1:n_refstar]
...
#    print('Instrument Mag of RefStars',mag_instrument_refstar)
#    print('Rmag of RefStars',rmag_refstar)    
    print()
#    shift,slope=polyfit(mag_instrument_refstar,rmag_refstar,1)

#    param,res=polyfit(mag_instrument_refstar,rmag_refstar,1,full=True)
    shift,slope=polyfit(mag_instrument_refstar,rmag_refstar,1,w=1/(mag_instrument_err[k][1:]))
#    residual=res[0].tolist()[0]
#    print('shift =','%.3f' %shift)
#    print('slope =','%.3f' %slope)
#    print('residual =', residual)

    mag_instrument_target=mag_instrument[k][0]
    rmag_target=shift+slope*mag_instrument_target
#    print(rmag_target)
...
#    if change ra,dec of reference star, run 'gasp_refStar_radec.py again.    
    norm = simple_norm(imdata, 'sqrt', percent=98)
...
#    plt.savefig(dir_obj+'Rmag_InsMag_aper_'+obj_name+'_'+date+'_fwhm'+str(xfwhm)+'_'+str(k)+'.png')
    plt.savefig(dir_obj+'Rmag_InsMag_aper_'+obj_name+'_annu_'+date+'_'+str(k)+'.png')
    plt.close()
...
#    plt.plot(mag_instrument_refstar,rmag_refstar,'o')
    rmag_fitting_source=shift+slope*mag_instrument[k]
    rmag_fitting_refstar=rmag_fitting_source[1:]
    print('... rmag_fitting_refstar:',rmag_fitting_refstar)
    d_rmag_fitting=rmag_fitting_refstar-rmag_refstar
    print('... fitting residual:',d_rmag_fitting)

#    rmag=shift+slope*mag_instrument
#    mag_instrument_fitting_refstar=(rmag_fitting_refstar-shift)/slope
...
#    print('measurement error (mag)',bkg_err[k])    
    SSregression_rmag=sum((rmag_fitting_refstar-np.mean(rmag[1:]))**2)
    SSresiduals_rmag=sum((d_rmag_fitting)**2)
    SStotal_rmag=sum((rmag[1:]-np.mean(rmag[1:]))**2)
    R_square_rmag[k]=SSregression_rmag/SStotal_rmag
    ...
    text_fitting='Rmag='+str('%.2f' %shift)+'+'+str('%.2f' %slope)+'*(Inst.Mag), $R^2$='+str('%.2f' %R_square_rmag[k])
    plt.plot(mag_instrument[k],rmag_fitting_source,'-',color='grey',label=text_fitting)
#    plt.plot(mag_instrument_refstar,rmag_refstar,'o',label='Ref.Star')
    plt.errorbar(mag_instrument_refstar,rmag_refstar,yerr=mag_instrument_err[k][1:],fmt='o',label='Reference Stars')
    label_obj=obj_name+', Rmag='+str('%.2f' %rmag_target)+'$\pm$'+str('%.2f' %err_mag_total_per_img[k])
    plt.plot(mag_instrument_target,rmag_target,'o',color='red',label=label_obj)
#    plt.errorbar(mag_instrument_target,rmag_target,yerr=mag_instrument_err[k][0],fmt='o',color='red',label=label_obj)
#    plt.errorbar(mag_instrument_source,rmag_source,yerr=mag_err[:-1])
    ...
    imin=min(mag_instrument[k])
    imax=max(mag_instrument[k])
...
df_out['ErrorCountsBGsub']=err_count_BGsub_per_img
df_out['ErrorCountsBGsub']=df_out['ErrorCountsBGsub'].map('{:.4f}'.format)
...
print(df_out)
file_Rmag_out=dir_obj+'Rmag_aperture_'+obj_name+'_annu.txt'
...
plt.errorbar(JD,Rmag_targets,yerr=err_mag_total_per_img,label='data',fmt='o')
#plt.plot(JD,Rmag_targets,'-')
plt.xlabel('JD',fontsize=font_small)
plt.ylabel('Rmag',fontsize=font_small)
plt.gca().invert_yaxis()
plt.title(obj_name,fontsize=font_small)
plt.xticks(fontsize=font_small)
plt.yticks(fontsize=font_small)
plt.legend(loc='best',fontsize=font_small)
#plt.show()
plt.savefig(dir_obj+'Rmag_JD_'+obj_name+'_annu.png')
plt.close()
...
plt.legend(loc='best',fontsize=font_small)
#plt.show()
plt.savefig(dir_obj+'Rmag_JD_'+obj_name+'_annu_line.png')
plt.close()
#sys.exit(0)
