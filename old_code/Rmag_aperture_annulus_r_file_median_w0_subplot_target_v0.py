...
dir_obj='Rmag_InstMag/annu_w0/'+obj_name+'/'
...
#    plt.savefig(dir_obj+'Rmag_InsMag_rmag_'+obj_name+'_annu_'+date+'_'+str(k)+'.png')
    plt.savefig(dir_obj+'Rmag_InsMag_'+obj_name+'_annu_'+date+'_'+str(k+1)+'.png',dpi=180)    
    plt.close()    
...
#del idx_fitsheader[idx_skip]
idx_fitsheader_drop=np.delete(idx_fitsheader,idx_skip)
print(idx_fitsheader_drop)
ID_drop=idx_fitsheader_drop+1
print(ID_drop)
idx_fitsheader_skip=idx_fitsheader[idx_skip]
ID_skip=idx_fitsheader_skip+1
print(ID_skip)
n_idx_drop=len(idx_fitsheader_drop)
n_skip=len(idx_skip)
print()
print('... skip idx_fitsheader_drop',idx_skip,'=',idx_fitsheader[idx_skip],'... bad fits ...')
print()


print('after drop bad fits: idx_fitsheader_drop =',idx_fitsheader_drop)
print('... keep fits ...')
print(fits_ori[idx_fitsheader_drop])    
        
#=======================
#sys.exit(0)
df_out_drop=df_out
df_out_drop=df_out_drop[~df_out_drop['ID'].isin(ID_skip)]
print(df_out_drop)
# -----------------------------------
df_Rmag_drop=df_out_drop[['JD','Rmag','ErrorRmag']].astype(float) #.reset_index(drop=True)
file_Rmag_out_drop=dir_obj+'Rmag_'+obj_name+'.txt'
df_Rmag_drop.to_csv(file_Rmag_out_drop,sep=',')
# -----------------------------------
#sys.exit(0)
...
#plt.scatter(JD,Rmag_targets,label='data')
JD_drop=df_Rmag_drop['JD'] #.tolist()
Rmag_targets_drop=df_Rmag_drop['Rmag'] #.tolist()
err_mag_total_per_img_drop=df_Rmag_drop['ErrorRmag'] #.tolist()
#plt.scatter(JD_drop,Rmag_targets_drop,marker='.',color='red') #,label=obj_name,fmt='.',color='red')

plt.errorbar(JD_drop,Rmag_targets_drop,yerr=err_mag_total_per_img_drop,label=obj_name,fmt='.',color='red')
...
#plt.show()
plt.savefig(dir_obj+'Rmag_JD_'+obj_name+'_RefStar_annu.png',dpi=120)
plt.close()
#sys.exit(0)
